...
import { Destination, NonWitnessUtxo, TxData, TxInputItem, OutputItem, WitnessUtxo } from './bitcoin.types';
...
export class PsbtBuilder {
  ...
  private verifyInput = (txData: TxData, disableLargeFee: boolean = true) => {
    ...
    if (this.isDestinationOutputs(txData.outputs)) {
      ...
      const totalOuputs = txData.outputs.reduce(
        (acc: number, cur: OutputItem) => acc + cur.value,
        0,
      );
  ...
  private isNonWitnessUtxo = (utxo: WitnessUtxo | NonWitnessUtxo): utxo is NonWitnessUtxo => {
    return (utxo as NonWitnessUtxo).nonWitnessUtxo !== undefined;
  };

  private isDestinationOutputs = (out: OutputItem[] | Destination): out is Destination => {
