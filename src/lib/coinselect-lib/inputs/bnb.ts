import { finalize, inputBytes, outputBytes, sumOrNaN, transactionBytes, uintOrNaN } from '../utils';

const maxTries = 1000000;

function calculateEffectiveValues(utxos, feeRate) {
  return utxos.map((utxo) => {
    if (Number.isNaN(uintOrNaN(utxo.value))) {
      return {
        utxo,
        effectiveValue: 0,
      };
    ...
    const effectiveFee = inputBytes(utxo) * feeRate;
...
export const branchAndBound = (factor) => {
  return (utxos, outputs, feeRate, options) => {
    ...
    const costPerChangeOutput = outputBytes({
      script: {
        length: changeOutputLength,
      },
    }) * feeRate;
    const costPerInput = inputBytes({
      script: {
        length: inputLength,
      },
    }) * feeRate;
    ...
    const outAccum = sumOrNaN(outputs) + transactionBytes([], outputs) * feeRate;
